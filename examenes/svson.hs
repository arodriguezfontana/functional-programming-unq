...
-- Restringe un objeto (representado por una funcion) a solo los fields especificados. ks es la lista de campos permitidos, f la funcion original. Si el campo que llega esta en f, entonces f k, sino devuelve Nothing
only :: [Field] -> (Field -> Maybe Value) -> Field -> Maybe Value
only ks f = \k -> if elem k ks then f k else Nothing 

-- Actualiza o agrega un field con un valor en un objeto (representado por una funcion). k es el campo a actualizar, v es su valor y f es la funcion original. Si el campo que llega es igual al que hay que actualizar, se develve el valor. Sino f k.
...
-- Devuelve hasta n objetos que cumplan la condición dada, junto con el número real de objetos encontrados. Debe priorizar los índices menores (recorrido in-order del BST).
